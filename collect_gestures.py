...
def main():
    # Tasks API hand tracker. Returns landmarks in the same shape the old
    # mp.solutions.Hands did, so frame_row() etc. are unchanged. Landmark
    # drawing is dropped here: the collector never needed the skeleton overlay
    # to do its job, and the Tasks API draws differently. The red border and
    # the per-class counts are the feedback that actually matters while
    # recording.
    tracker = HandTracker(
        max_num_hands=1,
    ...
    while True:
        ...
        # Tasks API needs a millisecond timestamp; VIDEO mode uses it to track
        # the hand across frames instead of re-detecting from scratch.
        hands_found = tracker.detect(frame, now * 1000.0)

        landmarks = None
        if hands_found:
            landmarks = hands_found[0]  # single-hand collector: take the first
